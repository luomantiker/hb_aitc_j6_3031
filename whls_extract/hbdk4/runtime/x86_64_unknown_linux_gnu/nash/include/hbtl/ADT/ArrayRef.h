...
// forked from llvm-project ArrayRef.h
#pragma once
...
template <typename T, typename U> class SmallVectorTemplateCommon;
} // namespace llvm

HBTL_NAMESPACE_BEGIN {
  template <typename T> class HBTL_NODISCARD ArrayRef;
  template <typename T> class HBTL_NODISCARD MutableArrayRef;
  ...
  public:
    ...
    /// Construct an empty ArrayRef from hbtl::nullopt.
    /*implicit*/ ArrayRef(hbtl::nullopt_t) {}
    ...
    template <typename U>
    /*implicit*/ ArrayRef(const llvm::SmallVectorTemplateCommon<T, U> &Vec) : Data(Vec.data()), Length(Vec.size()) {}
    /*implicit*/ ArrayRef(const llvm::ArrayRef<T> &Vec) : Data(Vec.data()), Length(Vec.size()) {}
...
#pragma GCC diagnostic pop
    ...
    /*implicit*/ ArrayRef(const llvm::SmallVectorTemplateCommon<U *, DummyT> &Vec,
    ...
    operator llvm::ArrayRef<T>() const { return llvm::ArrayRef<T>(Data, Data + Length); }
  ...
  /// This is intended to be trivially copyable, so it should be passed by
  /// value.
  template <typename T> class HBTL_NODISCARD MutableArrayRef : public ArrayRef<T> {
  public:
    using value_type = T;
    using pointer = value_type *;
    using const_pointer = const value_type *;
    ...
    /// Construct an empty MutableArrayRef from hbtl::nullopt.
    /*implicit*/ MutableArrayRef(hbtl::nullopt_t) : ArrayRef<T>() {}
    ...
    /*implicit*/ MutableArrayRef(llvm::SmallVectorImpl<T> &Vec) : ArrayRef<T>(Vec) {}

    /// Construct a MutableArrayRef from a llvm::MutableArrayRef.
    /*implicit*/ MutableArrayRef(llvm::MutableArrayRef<T> &Vec) : ArrayRef<T>(Vec) {}
  ...
  template <typename T> ArrayRef(const llvm::SmallVectorImpl<T> &Vec) -> ArrayRef<T>;

  /// Deduction guide to construct an ArrayRef from a SmallVector
  template <typename T, unsigned N> ArrayRef(const llvm::SmallVector<T, N> &Vec) -> ArrayRef<T>;

  /// Deduction guild to construct an ArrayRef from a llvm::ArrayRef
  template <typename T> ArrayRef(const llvm::ArrayRef<T> &Vec) -> ArrayRef<T>;

  /// Deduction guild to construct an ArrayRef from a llvm::ArrayRef
  template <typename T> ArrayRef(const llvm::MutableArrayRef<T> &Vec) -> ArrayRef<T>;
  ...
  template <class T> MutableArrayRef(llvm::SmallVectorImpl<T> & Vec) -> MutableArrayRef<T>;

  /// Deduction guild to construct an `MutableArrayRef` from a `llvm::MutableArrayRef`
  template <typename T> MutableArrayRef(const llvm::MutableArrayRef<T> &Vec) -> MutableArrayRef<T>;

  /// Deduction guide to construct a `MutableArrayRef` from a `SmallVector`.
  template <class T, unsigned N> MutableArrayRef(llvm::SmallVector<T, N> & Vec) -> MutableArrayRef<T>;
  ...
  template <typename T, size_t N> MutableArrayRef(T(&Arr)[N]) -> MutableArrayRef<T>;
  ...
  template <typename T> inline bool operator==(llvm::SmallVectorImpl<T> &LHS, ArrayRef<T> RHS) {
    return ArrayRef<T>(LHS).equals(RHS);
  }

  template <typename T> inline bool operator==(const llvm::ArrayRef<T> &LHS, const ArrayRef<T> RHS) {
    return ArrayRef<T>(LHS).equals(RHS);
  }

  template <typename T> inline bool operator==(const ArrayRef<T> &LHS, const llvm::ArrayRef<T> RHS) {
    return LHS.equals(ArrayRef<T>(RHS));
  }
  ...
  template <typename T> inline bool operator!=(llvm::SmallVectorImpl<T> &LHS, ArrayRef<T> RHS) { return !(LHS == RHS); }
  ...
  template <typename T> std::ostream &operator<<(std::ostream &os, const ArrayRef<T> Vec) {
    constexpr size_t maxDisplayElements = 10U;
    auto printer = [&os](auto i) {
      if HBTL_CONSTEXPR_IF (std::is_integral<decltype(i)>::value) {
        if HBTL_CONSTEXPR_IF (std::is_unsigned<decltype(i)>::value) {
          os << static_cast<uint64_t>(i);
        } else {
          os << static_cast<int64_t>(i);
        }
      } else {
        os << i;
      }
    };
    auto delimiter = [&os] { os << ","; };

    os << "[";
    if (Vec.size() > maxDisplayElements) {
      interleave(Vec.take_front(maxDisplayElements), printer, delimiter);
      os << ",...]";
    } else {
      interleave(Vec, printer, delimiter);
      os << "]";
    }
    return os;
  }
