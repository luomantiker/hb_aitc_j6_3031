...
#else
      if (pos >= size()) {
        throw std::out_of_range("hbtl::string_view::at()");
      }
#endif
      return data_at(pos);
    }
...
#else
      if (pos > size()) {
        throw std::out_of_range("hbtl::string_view::copy()");
      }
#endif
...
#else
      if (pos > size()) {
        throw std::out_of_range("hbtl::string_view::substr()");
      }
#endif
...
HBTL_NAMESPACE_BEGIN {
  nssv_inline_ns namespace literals {
    nssv_inline_ns namespace string_view_literals {
      ...
      nssv_constexpr hbtl::sv_lite::string_view operator"" sv(const char *str, size_t len) nssv_noexcept // (1)
      {
        return hbtl::sv_lite::string_view{str, len};
      }

      nssv_constexpr hbtl::sv_lite::u16string_view operator"" sv(const char16_t *str, size_t len) nssv_noexcept // (2)
      {
        return hbtl::sv_lite::u16string_view{str, len};
      }

      nssv_constexpr hbtl::sv_lite::u32string_view operator"" sv(const char32_t *str, size_t len) nssv_noexcept // (3)
      {
        return hbtl::sv_lite::u32string_view{str, len};
      }

      nssv_constexpr hbtl::sv_lite::wstring_view operator"" sv(const wchar_t *str, size_t len) nssv_noexcept // (4)
      {
        return hbtl::sv_lite::wstring_view{str, len};
      }
      ...
      nssv_constexpr hbtl::sv_lite::string_view operator"" _sv(const char *str, size_t len) nssv_noexcept // (1)
      {
        return hbtl::sv_lite::string_view{str, len};
      }

      nssv_constexpr hbtl::sv_lite::u16string_view operator"" _sv(const char16_t *str, size_t len) nssv_noexcept // (2)
      {
        return hbtl::sv_lite::u16string_view{str, len};
      }

      nssv_constexpr hbtl::sv_lite::u32string_view operator"" _sv(const char32_t *str, size_t len) nssv_noexcept // (3)
      {
        return hbtl::sv_lite::u32string_view{str, len};
      }

      nssv_constexpr hbtl::sv_lite::wstring_view operator"" _sv(const wchar_t *str, size_t len) nssv_noexcept // (4)
      {
        return hbtl::sv_lite::wstring_view{str, len};
      }
...
//
// make types and algorithms available in namespace hbtl:
//
...
template <> struct hash<hbtl::string_view> {
public:
  std::size_t operator()(hbtl::string_view v) const nssv_noexcept {
    return std::hash<std::string>()(std::string(v.data(), v.size()));
  }
};

template <> struct hash<hbtl::wstring_view> {
public:
  std::size_t operator()(hbtl::wstring_view v) const nssv_noexcept {
    return std::hash<std::wstring>()(std::wstring(v.data(), v.size()));
  }
};

template <> struct hash<hbtl::u16string_view> {
public:
  std::size_t operator()(hbtl::u16string_view v) const nssv_noexcept {
    return std::hash<std::u16string>()(std::u16string(v.data(), v.size()));
  }
};

template <> struct hash<hbtl::u32string_view> {
public:
  std::size_t operator()(hbtl::u32string_view v) const nssv_noexcept {
