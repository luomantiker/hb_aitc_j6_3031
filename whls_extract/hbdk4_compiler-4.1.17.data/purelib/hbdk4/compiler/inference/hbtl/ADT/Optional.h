...
#define TL_OPTIONAL_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(T) hbtl::detail::is_trivially_copy_constructible<T>::value
...
// in some cases. This is a check to workaround the common failing case.
  ...
  template <class... Ts> using void_tt = typename voider<Ts...>::type;

  // Trait for checking if a type is a hbtl::optional
  ...
  struct returns_void_impl<F, void_tt<invoke_result_t<F, U...>>, U...> : std::is_void<invoke_result_t<F, U...>> {};
...
namespace std {
// TODO SFINAE
template <class T> struct hash<hbtl::optional<T>> {
  ::std::size_t operator()(const hbtl::optional<T> &o) const {
    if (!o.has_value())
      return 0;

    return std::hash<hbtl::detail::remove_const_t<T>>()(*o);
  }
};
} // namespace std
