...
from hbdk4.compiler.eval_kit.model_gallery.utils import Backbone
...
class GroupConv2D(nn.Module):
    def __init__(
        self, in_channels, out_channels, kernel_size=1, n_chunks=1, bias=False
    ):
    ...
    def forward(self, x):
        if self.n_chunks == 1:
            ...
            out = torch.cat(
                [layer(s) for layer, s in zip(self.group_layers, split)], dim=1
            )
            return out
    ...
    def __init__(
        ...
        if expand_ratio != 1:
            self.expand_conv = nn.Sequential(
                GroupConv2D(
                    in_channels, in_channels * expand_ratio, n_chunks=expand_ksize
                ),
            ...
            if self._has_se:
                ...
                self.squeeze_excitation = SqueezeExcitation(
                    in_channels * expand_ratio, num_reduced_filters, swish
                )
            self.project_conv = nn.Sequential(
                GroupConv2D(
                    in_channels * expand_ratio, out_channels, n_chunks=project_ksize
                ),
                nn.BatchNorm2d(out_channels),
            )
        else:
            ...
            if self._has_se:
                num_reduced_filters = max(1, int(in_channels * se_ratio))
                self.squeeze_excitation = SqueezeExcitation(
                    in_channels, num_reduced_filters, swish
                )
            self.project_conv = nn.Sequential(
    ...
    def __init__(
        ...
        if dataset == "IMAGENET":
            self.conv = nn.Sequential(
                nn.Conv2d(
                    in_channels=3, out_channels=stem, kernel_size=3, stride=2, padding=1
                ),
                nn.BatchNorm2d(stem),
                nn.ReLU(),
            )
            ...
            self.conv = nn.Sequential(
                nn.Conv2d(
                    in_channels=3, out_channels=stem, kernel_size=3, stride=1, padding=1
                ),
                nn.BatchNorm2d(stem),
                nn.ReLU(),
            )
...
def mixnet_s(
    num_classes=1000, multiplier=1.0, divisor=8, min_depth=None, dataset="IMAGENET"
):
    if dataset == "IMAGENET":
        small = [
...
def genMixNet(backbone, *sizes):
    ...
    """
    ei = torch.rand(*sizes)
    net = Backbone(globals()[backbone](), permute_return=True)
    traced = trace(net, ei, splat=True)
    print("run mixnet", backbone, "example input", ei.shape)
    return export(traced, ei, name=backbone)
