...
class Upsample_interpolate(nn.Module):
    ...
    def forward(self, x):
        assert x.data.dim() == 4
        out = F.interpolate(x, scale_factor=self.stride, mode="nearest")
        return out
...
class Reorg(nn.Module):
    ...
    def forward(self, x):
        return torch.cat([x[..., ::2, ::2], x[..., 1::2, ::2]], 1)
...
class Darknet(nn.Module):
    def __init__(self, cfgfile, post_process=True):
        ...
        for block in blocks:
            if block["type"] == "net":
                ...
                if activation == "leaky":
                    ...
                    print("unreachable convolution activation {}".format(activation))
