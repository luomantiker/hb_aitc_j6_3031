...
from horizon_plugin_pytorch.functional import batched_nms
...
def bgr_to_yuv444(input: Tensor, channel_reversal: bool) -> Tensor:
    ...
    """
    return torch.ops.horizon.bgr_to_yuv444(input, channel_reversal)
...
def max_iou_match(
    ...
    Match boxes to gt_boxes according to corresponding overlap type
    and threshold.
    ...
    """
    return torch.ops.horizon.max_iou_match(
        boxes,
        gt_boxes,
        gt_boxes_num,
        pos_iou,
...
def ig_region_match(
    ...
    Returns:
        Output flag with shape [B, N, C - 1] if exclude_background else
            [B, N, C]
    """
    return torch.ops.horizon.ig_region_match(
        boxes,
        ig_boxes,
        ig_boxes_num,
        class_num,
...
def point_pillars_scatter(
    ...
    if has_torch_function_unary(voxel_features):
    ...
    return quantized_point_pillars_scatter(
        voxel_features, coords, output_shape
    )
...
def window_partition(x: Union[Tensor, QTensor], window_size: int):
    ...
    if isinstance(x, QTensor):
        ...
        return quantized_window_partition(x, window_size)
...
def window_reverse(
    ...
    if isinstance(windows, QTensor):
        ...
        return quantized_window_reverse(windows, window_size, h, w)
...
def rle(input: Union[Tensor, QTensor], dtype: torch.dtype) -> List[Tensor]:
    ...
    Compress the tensor in the format of
    ...
    if has_torch_function_unary(input):
        return handle_torch_function(rle, (input,), input, dtype)
...
def point_pillars_preprocess(
    ...
    Returns:
        (Tensor, Tensor): (features, coords), encoded feature
        and coordinates in (idx, z, y, x) format.
    """
    if has_torch_function(points_list):
        return handle_torch_function(
            point_pillars_preprocess,
            points_list,
            points_list,
...
@handle_autocast(
    supported_cpu_dtype=(torch.float16,), supported_gpu_dtype=(torch.float16,)
)
def clamp(input: torch.Tensor, min_value: float, max_value: float):
    return torch.ops.horizon.clamp(input, min_value, max_value)


@fx_helper.wrap()
def relu(input: torch.Tensor):
    return clamp(input, 0.0, float("inf"))


@fx_helper.wrap()
def relu6(input: torch.Tensor):
