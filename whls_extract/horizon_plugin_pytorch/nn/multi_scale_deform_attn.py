...
class MultiScaleDeformableAttention(torch.nn.Module):
    ...
    def __init__(
        ...
        # which is more efficient in its CUDA implementation
    ...
    def forward(
        ...
        if torch.cuda.is_available() and value.is_cuda:
            output = torch.ops.horizon.ms_deform_attn(
                value,
                spatial_shapes,
                torch.cat(
                    (
                        torch.tensor([0]).to(spatial_shapes.device),
                        torch.cumsum(
                            spatial_shapes[:, 1] * spatial_shapes[:, 0], dim=0
                        )[:-1],
                    ),
                    dim=0,
                ).long(),
