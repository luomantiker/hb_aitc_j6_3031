...
class RoIAlign(torch.nn.Module):
    ...
    @typechecked
    def forward(
        self, featuremaps: QTensor, rois: Union[List[Tensor], QTensor, Tensor]
    ) -> QTensor:
        """
        ...
        if featuremaps.dtype != self.activation_post_process.dtype:
            old_dtype = self.activation_post_process.dtype
            self.activation_post_process.reset_dtype(featuremaps.dtype, False)
            logger.warning(
                f"{self.__class__.__name__} output dtype {old_dtype} "
                f"will be changed to {featuremaps.dtype}.",
                extra={"call_times_context": ("message", "location")},
            )
        ...
        if isinstance(rois, list):
            if isinstance(rois[0], QTensor):
                ...
                # https://gitlab.hobot.cc/ptd/ap/dlp/
                # horizon_plugin_pytorch/issues/67
            ...
            out = roi_align_list(
                ...
                featuremaps.dtype,
                roi_quantized,
                getattr(self, "quantized_aligned_qat", False),
            )
        else:
