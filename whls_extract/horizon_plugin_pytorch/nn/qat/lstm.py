...
from horizon_plugin_pytorch.dtype import _storage_type_map
...
class LSTM(torch.nn.Module):
    ...
    def __init__(
        ...
        for layer_i in range(num_layers):
            ...
            if self.dropout > 0:
                self.add_module("dropout" + sfx, torch.nn.Dropout(dropout))
            if bidirectional:
                sfx += "_reverse"
                self._add_common_module(sfx, layer_i)
    ...
    def _add_common_module(self, sfx, layer_i):
        ...
        self.add_module("cat" + sfx, FloatFunctional(qconfig=self.qconfig))
        if self.proj_size > 0:
            self.add_module(
                "proj_linear" + sfx,
                Linear(
    ...
    def _lstm_cell(self, i2h, hid_state, cell_state, sfx):
        ...
        split_gates = torch.split(
            gate, split_size_or_sections=self.hidden_size, dim=2
        )
    ...
    def _lstm(
        ...
        for seq_i in range(sequence_length):
            ...
            if backward:
                i2h = i2hs[sequence_length - seq_i - 1]
            else:
                i2h = i2hs[seq_i]
    ...
    @typechecked
    def forward(self, data: QTensor, states=None):
        ...
        if self.bidirectional:
            for layer_i in range(self.num_layers):
                f_output = []
                b_output = []
                f_hid_state = hid_state[2 * layer_i : 2 * layer_i + 1]
                f_cell_state = cell_state[2 * layer_i : 2 * layer_i + 1]
                b_hid_state = hid_state[2 * layer_i + 1 : 2 * layer_i + 2]
                b_cell_state = cell_state[2 * layer_i + 1 : 2 * layer_i + 2]
                ...
                out_hstates.append(
                    getattr(self, "cat_fbhs" + str(layer_i) + "_reverse").cat(
                        [f_hid_state, b_output[0]], dim=0
                    )
                )
                out_cstates.append(
                    getattr(self, "cat_fbcs" + str(layer_i) + "_reverse").cat(
                        [f_cell_state, b_cell_state], dim=0
                    )
                )
                f_res = getattr(self, "cat" + str(layer_i)).cat(
                    f_output, dim=0
                )
                b_res = getattr(self, "cat" + str(layer_i) + "_reverse").cat(
                    b_output, dim=0
                )
                data = getattr(self, "cat_fb" + str(layer_i)).cat(
                    [f_res, b_res], dim=2
                )
            ...
            for layer_i in range(self.num_layers):
                hstate = hid_state[layer_i : layer_i + 1]
                cstate = cell_state[layer_i : layer_i + 1]
                seq_out = []
                ...
                data = getattr(self, "cat" + str(layer_i)).cat(seq_out, dim=0)
                if self.dropout > 0 and layer_i < self.num_layers - 1:
                    data = getattr(self, "dropout" + str(layer_i))(data)
        out = data
        hid_state = self.cat_seq_hs.cat(out_hstates, dim=0)
        cell_state = self.cat_seq_cs.cat(out_cstates, dim=0)
        if self.batch_first:
