...
class Correlation(nn.Module):
    r"""Perform multiplicative patch comparisons between two feature maps.
    ...
    :math:`c(x_{1}, x_{2}) = \displaystyle\sum_{o \in [-k,k] \times [-k,k]} <f_{1}(x_{1} + o), f_{2}(x_{2} + o)>`
    ...
    def _forward(
        ...
        inter_width = (top_width - 1) * strides[0] + kernel
        ...
        self.sumelems = kernel * kernel * channels
    ...
    def forward(
        self,
        data1: Union[torch.Tensor, QTensor],
        data2: Union[torch.Tensor, QTensor],
    ) -> torch.Tensor:
