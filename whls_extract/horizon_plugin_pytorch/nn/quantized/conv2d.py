...
from .functional import conv2d, conv2d_convert_int_params

__all__ = ["Conv2d", "ConvReLU2d", "ConvAdd2d", "ConvAddReLU2d"]


def _bernoulli_bias_quantization(qconv, bias):
    m, e = torch.ops.horizon.frexp(torch.clamp(bias, -128, 127))
...
def _fuse_bn_to_wscale(qconv, mod, march, weight_post_process):
    ...
    if march == March.BERNOULLI:
        ...
        m, e = torch.ops.horizon.frexp(quantized_weight)
...
class Conv2d(Module):
    r"""Applies a 2D convolution.

    Applies a 2D convolution over a quantized input signal
    ...
    def forward(
        ...
        if hasattr(self, "fast_mode") and self.fast_mode:
            torch._C._log_api_usage_once("quantized conv fast forward")
            device = input1.device
            key = tuple(
                (
                    torch.cat((input1.q_scale(), input2.q_scale()))
                    if input2 is not None
                    else input1.q_scale()
                ).tolist()
            )
            if key not in self.qparams_dict:
                self.qparams_dict[key] = conv2d_convert_int_params(
                    input1.q_scale(),
                    self.weight,
                    self.weight_scale,
                    self.weight_dtype,
                    ...
                    self.out_scale,
                    self.out_dtype,
                    input2.q_scale() if input2 is not None else None,
                    False,
                    1,
                )
            (
                ...
                bpu_output_rshift,
                dequant_out_scale,
            ) = self.qparams_dict[key]
            out = torch.ops.horizon.bpu_scale_quanti_convolution(
                input1.int_repr(),
                bpu_weight.to(device),
                bpu_bias.to(device),
                input2.int_repr()
                if input2 is not None
                else torch.zeros(1).to(input1.int_repr()),
                bpu_oscale.to(device),
                bpu_accu_rshift.to(device),
                bpu_bias_lshift.to(device),
                bpu_output_rshift.to(device),
                bpu_escale.to(device),
                bpu_escale_lshift.to(device),
                True,  # use_bias,
                self.weight.size()[0],
                ...
                self.padding,
                self.dilation,
                self.activation if hasattr(self, "activation") else "",
                self.groups,
                True if input2 is not None else False,
                True if self.out_dtype == "qint32" else False,
                self.out_dtype,
                get_march(),
            )
        else:
            torch._C._log_api_usage_once("quantized conv normal forward")
    ...
    def from_float(cls, mod):
        ...
        if hasattr(mod, "_swap_inputs"):
            qconv.swap_inputs(mod._swap_inputs)

        return qconv


class ConvReLU2d(Conv2d):
    r"""A ConvReLU2d module is a fused module of Conv2d and ReLU.
...
class ConvAdd2d(Conv2d):
    r"""A ConvAdd2d module is a fused module of Conv2d and add.
