...
def linear_transform(
    ...
    km, ke = torch.ops.horizon.frexp(k)
    shift = bits - ke.item()
    if shift > max_shift:
        retk = 0
        bm, be = torch.ops.horizon.frexp(b)
        shift = bits - be.item()
...
def _get_lut_params(
    ...
    itplt_shift = 4
    left_shift = 0
    m, k = torch.ops.horizon.frexp(torch.tensor(float(bias)))
...
def _lut_with_int16(
    ...
    res = torch.ops.horizon.bpu_quanti_lut(
...
def _get_multi_table_params(
    ...
    """Provide params of line fit, constant fit and table fit for compiler.
...
def line_fit_interval(
    ...
    res = torch.ops.horizon.bpu_quanti_line_fit(
