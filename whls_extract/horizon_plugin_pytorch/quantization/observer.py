...
# Kernel fusion can be observed after several forward
# https://github.com/pytorch/pytorch/issues/76799
@torch.jit.script
...
class _ObserverBase(ObserverBase):
    ...
    def __init__(
        ...
        if (quant_min is not None) and (quant_max is not None):
            assert (
                quant_min < quant_max
            ), "qmin must be strictly less than qmax for user-specified quantization range."  # noqa: E501
            assert (
    ...
    def _calculate_qparams(
        ...
        Given min and max value tensors. Works for both per tensor
        and per channel cases
        ...
        if min_val.numel() == 0 or max_val.numel() == 0:
            warnings.warn(
                "must run observer before calling calculate_qparams. Returning default scale and zero point",  # noqa: E501
            )
...
class MovingAverageMinMaxObserver(MinMaxObserver):
    r"""MovingAverageMinMax Observer.
...
class PerChannelMinMaxObserver(_ObserverBase):
    ...
    def forward(self, x_orig):
        ...
        min_val_cur, max_val_cur = torch.aminmax(x.flatten(start_dim=1), dim=1)
...
class MovingAveragePerChannelMinMaxObserver(PerChannelMinMaxObserver):
    r"""MovingAveragePerChannelMinMax Observer.
...
class ClipObserver(_ObserverBase):
    ...
    def forward(self, x_orig):
        ...
        min_val_cur, max_val_cur = torch.aminmax(x)
        self.min_val = min_val_cur.reshape(-1).clamp_min_(self.setted_min)
        self.max_val = max_val_cur.reshape(-1).clamp_max_(self.setted_max)
...
class CalibObserver(torch.nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer("_calib_bin_edges", torch.tensor([]))
        self.register_buffer("_calib_hist", torch.tensor([]))
        self._method = method
        self._device = None
        ...
        if axis is not None:
            raise NotImplementedError(
                "Calibrator histogram collection only supports per tensorscaling"  # noqa: E501
            )
    ...
    def _load_from_state_dict(
        ...
        local_state = ["_calib_bin_edges", "_calib_hist"]
        for name in local_state:
            key = prefix + name
            if key in state_dict:
                val = state_dict[key]
                if name == "_calib_bin_edges":
                    self._calib_bin_edges.resize_(val.shape)
                else:
                    self._calib_hist.resize_(val.shape)

        super(CalibObserver, self)._load_from_state_dict(
            state_dict,
            prefix,
            local_metadata,
            strict,
    ...
    def collect(self, x):
        ...
        with torch.no_grad():
            ...
            if x_max == 0:
                logger.warning(
                    "The inputs of this layer are all 0, check you input and "
                    "model",
    ...
    def reset(self):
        ...
        self._calib_bin_edges.resize_(0)
        self._calib_hist.resize_(0)
    ...
    def __str__(self):
        s = "CalibObserver("
        if self._calib_bin_edges.numel() == 0:
            bin_edge_str = "None"
        else:
    ...
    def __repr__(self):
        ...
        return s.format(
            _calib_bin_edges=self._calib_bin_edges,
            _calib_hist=self._calib_hist,
        )
    ...
    def _compute_amax_entropy(
        ...
        for i in range(starting, stop + 1, stride):
            ...
            if (
                ...
                raise RuntimeError(
                    "Count mismatch! total_counts_new={}, total_counts_old={}, total_data={}".format(  # noqa: E501
