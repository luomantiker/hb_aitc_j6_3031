...
from horizon_plugin_pytorch.fx.fx_helper import get_node_used_count
...
def is_match(modules, node, pattern, max_uses=sys.maxsize):
    """Match a node in fx against a pattern."""
    ...
    if isinstance(self_match, type) and issubclass(
        ...
        if arg_matches:
            ...
            if isinstance(arg_matches[0], str):
                ...
                return isinstance(arg_matches[0], type(modules[node.target]))
            else:
        ...
        if owner is None or owner is self_match:
            ...
            use_count = get_node_used_count(node)
