    ...
    def __init__(
        ...
        if self.use_dense:
            ...
            for _ in range(len(self.mod)):
                self.concat.append(hpp.nn.quantized.FloatFunctional())
        ...
        if self.use_res:
            ...
            self.short_add = hpp.nn.quantized.FloatFunctional()
...
class PwcNetHead(nn.Module):
    ...
    @fx_wrap()
    def warp(
        self, x: torch.Tensor, up_flow: torch.Tensor, idx: int
    ) -> torch.Tensor:
        """
    ...
    def forward(
        ...
        for idx in range(self.pyr_lvls, self.flow_pred_lvl - 1, -1):
            if idx == self.pyr_lvls:
                ...
                feat1_warp = self.warp(feat1[idx - 1], up_flow, idx)
                ...
                if idx == self.flow_pred_lvl:
                    ...
                    flow_list.append(flow)
        return self._dequant(flow_list)

    @fx_wrap()
    def _dequant(self, flow_list):
        if self.training:
