...
def get_colormap(task, desc_id):
    if task == "default_parsing":
        labels = get_default_parsing_labels(desc_id)
    elif task == "image_fail_parsing":
        labels = _get_image_fail_parsing_labels(desc_id)
    elif task == "lane_parsing":
        labels = _get_lane_parsing_labels(desc_id)
    colormap = [label[ParsingDescColorMap] for label in labels]
    colormap = {i: c for i, c in enumerate(colormap)}
    colormap[255] = [0, 0, 0]
    return colormap


def _get_lane_parsing_labels(desc_id: str) -> List[Dict]:
