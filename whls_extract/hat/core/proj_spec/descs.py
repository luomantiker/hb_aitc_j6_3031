...
def frcnn_roi_det_with_subscore_desc(
    ...
    if box_clip_ratio is None:
        desc_offset.pop("box_clip_ratio")
...
def get_crop_desc(
    ...
    norm_len: int,
    norm_method: str,
    padding_context: Optional[Union[List, Tuple]] = None,
...
def get_default_parsing_desc(
    ...
    """
    label_desc = get_parsing_desc_template(
        task_name="parsing",
    ...
    if not with_rle:
        ...
        score_desc = label_desc.replace(
            '"task": "parsing"',
            '"task": "parsing_score"',
        )
        rle_desc = label_desc.replace(
            '"task": "parsing"',
            '"task": "parsing_rle"',
        )
        return [score_desc, label_desc, rle_desc]
...
def get_lane_parsing_desc(
    ...
    if not with_rle:
        ...
        score_desc = label_desc.replace(
            '"task": "lane_parsing"',
            '"task": "lane_parsing_score"',
        )
        rle_desc = label_desc.replace(
            '"task": "lane_parsing"',
            '"task": "lane_parsing_rle"',
        )
...
def get_image_fail_parsing_desc(
    ...
    if not with_rle:
        ...
        score_desc = label_desc.replace(
            '"task": "image_fail_parsing"',
            '"task": "image_fail_parsing_score"',
        )
        rle_desc = label_desc.replace(
            '"task": "image_fail_parsing"',
            '"task": "image_fail_parsing_rle"',
        )
...
def position_encoding_desc(
    task_name: str,
    shape: Tuple,
) -> str:
    """Get __desc__ descriptor for input Position encoding.

    Args:
        task_name (str): task name for input PE
        shape (tuple): the shape of input PE
