...
def get_affine_transform(
    ...
    Args:
        ...
        rotation: rotation degree in angle degree
        out_size: Width and height of the output image
        inverse: If True, the matrix is from output image to input image
        center_shift: coord shift(x, y) of center point in
            input image, x/y could be positive or negative, in pixel
        pre_resize_scale: Default is -1.0.
...
def point_2D_affine_transform(
    data: np.ndarray, affine_mat: np.ndarray
) -> np.ndarray:
...
def bbox_affine_transform(
    data: np.ndarray, affine_mat: np.ndarray
) -> np.ndarray:
    """Transform bounding boxes by affine matrix.

    Args:
        data: Bounding boxes to apply transformation.
    ...
    Returns:
        new_bbox: The bounding boxes after transformation with shape (N, 4+)
    """
    assert isinstance(data, np.ndarray)
    if affine_mat.shape == (3, 3):
        affine_mat = affine_mat[0:2]
...
def get_vcs2bev_img_mat(vcs_range: Sequence, bev_size: Sequence):
    ...
    vcs2bev_img = np.array(vcs2bev_img, dtype="float32").reshape((3, 3))
