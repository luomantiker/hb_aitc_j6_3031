...
def get_affine_by_roi(
    src_wh: Tuple[int],
    target_wh: Tuple[int],
    src_roi: Union[Tuple, np.ndarray],
    target_roi: Union[Tuple, np.ndarray],
    center_aligned: bool = True,
    rand_scale_range: Tuple[int] = (1, 1),
    rand_translation_ratio: float = 0,
    rand_aspect_ratio: float = 0,
    rand_rotation_angle: int = 0,
    rand_shear_ratio: float = 0,
    flip_prob: float = 0,
):
...
def get_affine_roi_center_scale(
    src_wh: Tuple[int],
    target_wh: Tuple[int],
    roi_center_xy: Tuple[float],
    roi_scale_xy: Tuple[float],
    **kwargs
) -> AffineAugMat:
...
def resize_affine_mat(src_wh: Tuple[int, int], dst_wh: Tuple[int, int]):
    """Get the affine matrix for resize.

    Args:
        src_wh: The source width and height of image
        dst_wh: The target width and height of image
    """
    assert len(src_wh) == 2
    assert len(dst_wh) == 2
...
class AffineMat2DGenerator(object):
    ...
    @staticmethod
    def scale(x: Union[int, float], y: Union[int, float]):
        """Return the scaling matrix of affine.

        Args:
            x: The scale of x axis.
            y: The scale of y axis.
        """
        assert isinstance(x, numeric_types)
        assert isinstance(y, numeric_types)
    ...
    @staticmethod
    def resize(
        src_shape: Tuple, dst_shape: Tuple, pad_to_keep_ratio: bool = False
    ):
        """Return the resize matrix of affine.

        Args:
            src_shape: Source shape in hw/hwc format.
            dst_shape: Destinate shape in hw/hwc format.
            pad_to_keep_ratio: Whether to do padding to keep ratio.
        """
        r_h, r_w = src_shape[:2]
        d_h, d_w = dst_shape[:2]
        Rs = np.eye(3)
        if pad_to_keep_ratio:
            C = np.eye(3)
            C[0, 2] = -r_w / 2
            C[1, 2] = -r_h / 2
    ...
    @staticmethod
    def flip_with_axis(
        center_xy: Union[Tuple[int], List[int]],
        with_x: bool = True,
        with_y: bool = False,
    ):
    ...
    @staticmethod
    def rotate(
        center_xy: Tuple["numeric_types", "numeric_types"],
        angle: "numeric_types",
        scale: "numeric_types" = 1,
    ):
        """Return the rotation matrix of affine. The same as cv2.getRotationMatrix2D.

        Args:
            center_xy: The rotation center
            angle: The angle of rotation.
                Positive value means anti-clock rotation.
            scale: Default=1. Isotropic scale factor.
        """
    ...
    def shear(
        ...
        Args:
            center_xy: The center
            s_x: The shear ratio along x axis
            s_y: The shear ratio along y axis
        """
    ...
    @staticmethod
    def aspect_ratio_aug(max_x_ratio: numeric_types = 0):
    ...
    @staticmethod
    def resize_by_min_edge(src_wh: Tuple[int], target_min_edge: numeric_types):
        ...
        Args:
            src_wh: The source image width and height
            target_min_edge: The target minimal edge length
        """
        assert len(src_wh) == 2
    ...
    @staticmethod
    def resize_by_min_max(
        src_wh: Tuple[int], min_edge: numeric_types, max_edge: numeric_types
    ):
        ...
        Args:
            src_wh: The source image width and height
            min_edge: The target min edge length
            max_edge: The target max edge length
        """
    ...
    @staticmethod
    def resize_by_max_edge(src_wh: Tuple[int], target_max_edge: numeric_types):
        ...
        Args:
            src_wh: The source image width and height
            target_max_edge: The target max edge length
        """
        assert len(src_wh) == 2
    ...
    @staticmethod
    def getAffineTransform(pts_before: np.ndarray, pts_after: np.ndarray):
        """Calculate an affine transform from three pairs of the corresponding points.

        Args:
            pts_before: The points before transformation.
                It should be in the shape of (3, 2)
            pts_before: The points after transformation.
                It should be in the shape of (3, 2)
        """
    ...
    @staticmethod
    def stack_affine_transform(*mats: List[np.ndarray]) -> np.ndarray:
...
class AffineMatFromROIBoxGenerator(object):
    ...
    def __init__(
        self,
        target_wh: Union[Tuple[int, int], List[int]],
        scale_range: Union[Tuple[float, float], List[float]] = (
            1.0 / 32,
            4.0,
        ),
        min_sample_num: int = 1,
        max_sample_num: int = 5,
        min_valid_edge: int = 1,
        min_valid_area: int = 8,
        rand_sampling_bbox: bool = True,
        reselect_ratio: int = 0,
        **kwargs: Dict
    ):
    ...
    def __call__(self, bboxes, img_wh):
        ...
        for bbox_id in bbox_idx_list:
            ...
            # TODO(alan): bbox_label is useless here..
...
class ImageAffineTransform(object):
    ...
    def __init__(
        ...
        pyramid_min_step: float = 0.45,
        pyramid_max_step: float = 0.8,
        pixel_center_aligned: bool = True,
    ):
    ...
    def __call__(
        self,
        image: Union["mx.nd.NDArray", np.ndarray, "AlphaImagePyramid"],  # noqa
        ...
        new_affine_mat = affine_mat.astype(np.float64)
        ...
        if self._use_pyramid or is_pyramid_image:
            ...
            if cur_scale < 1:
                ...
                new_affine_mat = np.dot(
                    AffineMat2DGenerator.to3x3(affine_mat.astype(np.float64)),
                    affine_inv,
                )
                ret = cv2.warpAffine(
                    new_img,
...
class AlphaImagePyramid(object):
    ...
    def build_from(self, scale: float, inter_method: int = 1):
    ...
    def get_layer_by_scale(self, scale: float):
        """Given a scale.
...
class SwapPoints(object):
    ...
    def __call__(self, data: np.ndarray) -> np.ndarray:
        ...
        Args:
            data: Points to apply transformation.
                It should be in the shape of (I, N, D).

        Returns:
            new_data: Points after transformation.
        """
...
class LabelAffineTransform(object):
    # TODO(Alan): Very very inefficient implementation!!!
...
class Point2DAffineTransform(object):
    ...
    def __call__(self, data: np.ndarray, affine_mat: np.ndarray) -> np.ndarray:
...
def rand_scale_affine_mat(
    ...
    if dst_wh is None:
        dst_wh = src_wh
    assert len(dst_wh) == 2
    dst_wh = np.array(dst_wh).astype(np.int64)
    tar_wh = np.array(src_wh, dtype=np.float64) * np.random.uniform(
        min_scale, max_scale
    )
