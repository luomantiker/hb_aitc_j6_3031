...
class ScaleTransformer(Transformer):
    def __init__(self, scale_value, data_format="CHW"):
        self.scale_value = scale_value
        self.data_format = data_format
        super(ScaleTransformer, self).__init__()

    def run_transform(self, data):
        if isinstance(self.scale_value, (float, int)):
            data = data * self.scale_value
        else:
            if self.data_format == "HWC":
                data = data * self.scale_value
            else:
                data = data * self.scale_value[:, np.newaxis, np.newaxis]
        data = data.astype(np.float32)
        return data


class NormalizeTransformer(Transformer):
    def __init__(self, std, data_format="CHW") -> None:
        self.std = std
        self.data_format = data_format
    ...
    def run_transform(self, data):
        if isinstance(self.std, (float, int)):
            ...
            if self.data_format == "HWC":
                ...
                data = data / self.std[:, np.newaxis, np.newaxis]
        return data
...
class ResizeTransformer(Transformer):
    ...
    def run_transform(self, data):
        if self.resize_mode == 'skimage':
            ...
            raise ValueError("unsupport resize mode:{}.(skimage and opencv"
                             " are supported)".format(self.resize_mode))
...
class ShortSideResizeTransformer(Transformer):
    def __init__(self, short_size, data_type='float', interpolation=""):
        ...
        if interpolation != "" and \
                interpolation not in INTERPOLATION_DICT.keys():
            raise ValueError(
                "The transformer does not supported interpolation method "
                f"{interpolation} at the moment ")
        self.interpolation = interpolation
...
class _ColorConvertTransformer(Transformer):
    ...
    def run_transform(self, image) -> np.ndarray:
        image = image + self.source_offset
...
class YUV4442RGBTransformer(Transformer):
    def __init__(self, data_format):
...
class YUV4442BGRTransformer(Transformer):
    def __init__(self, data_format):
