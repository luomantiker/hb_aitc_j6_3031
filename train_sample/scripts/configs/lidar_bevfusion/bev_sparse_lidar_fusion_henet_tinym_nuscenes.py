...
task_name = "bev_sparse_lidar_fusion_henet_tinym_nuscenes"
num_classes = 10
batch_size_per_gpu = 4
dataloader_workers = 1
device_ids = [0, 1, 2, 3, 4, 5, 6, 7]  # 1 node
ckpt_dir = "./tmp_models/%s" % task_name
pretrain_ckpt_dir = "./tmp_models/bev_sparse_henet_tinym_nuscenes"
...
deploy_model = dict(
    type="SparseBevFusionOE",
    compiler_model=True,
    lidar_net=copy.deepcopy(lidar_network),
    lidar_level_idx=[1, 2],
    backbone=dict(
        type="HENet",
        in_channels=3,
...
deploy_model["lidar_net"].pop("pre_process")
...
calibration_data_loader.pop(
    "batch_sampler"
)  # Calibration do not support DDP or DP
...
hbir_infer_model = dict(
    type="SparseBEVFusionOEIrInfer",
    first_frame_input=first_frame_input,
    projection_mat_key="lidar2img",
    gobel_mat_key="lidar2global",
    lidar_preprocess=model["lidar_net"]["pre_process"],
    ir_model=dict(
        type="HbirModule", model_path=os.path.join(ckpt_dir, "quantized.bc")
    ),
    decoder=dict(type="SparseBEVOEDecoder"),
...
def prepare_inputs(infer_inputs):
    ...
    for _, frame in enumerate(dir_list):
        ...
        data["points"] = np.load(os.path.join(frame_path, "lidar.npy"))
...
def process_inputs(data, transforms=None):
    ...
    vis_inputs["points"] = model_input["points"]
...
def inputs_save_func(data, save_path):
    ...
    for idx_, sample_data in enumerate(data):
        ...
        np.save(os.path.join(save_dir, "lidar.npy"), sample_data["points"])
...
quant_analysis_solver = dict(
    ...
    device_id=2,
    dataloader=copy.deepcopy(val_data_loader),
    num_steps=100,
